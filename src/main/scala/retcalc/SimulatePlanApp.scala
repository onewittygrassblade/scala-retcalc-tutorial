...
object SimulatePlanApp extends App {
  ...
  def strMain(args: Array[String]): Validated[String, String] = {
    if (args.length != 6)
      ...
      (vFromUntil, vNbOfYearsSaving, vParams).tupled
        .andThen {
  ...
  def parseParams(args: Array[String]): RetCalcResult[RetCalcParams] =
    (
      parseInt("nbOfYearsInRetirement", args(2)),
      parseInt("netIncome", args(3)),
      parseInt("monthlyExpenses", args(4)),
      parseInt("initialCapital", args(5))
    ).mapN {
      case (nbOfYearsInRetirement, netIncome, monthlyExpenses, initialCapital) =>
        RetCalcParams(
